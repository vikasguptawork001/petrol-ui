...
const Nozzles = () => {
  ...
  const [reportNozzle, setReportNozzle] = useState(null);
  ...
  const openReport = (n) => {
    setReportNozzle(n);
    setShowReport(true);
    fetchReport(n.id, reportFromDate, reportToDate);
  };

  const fetchReport = async (nozzleId, from, to) => {
    try {
                  ...
                  <tr key={n.id} style={{ borderBottom: '1px solid #2a3340' }}>
                    ...
                    <td style={{ padding: '8px 10px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: '6px', justifyContent: 'center' }}>
                        <button onClick={() => openReport(n)} style={{ padding: '4px 8px', background: '#1d9e75', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', display: 'flex', alignItems: 'center', gap: '4px', color: '#fff' }}>
                          📊 Report
                        </button>
          ...
          <div style={{ ...modalContent, maxWidth: '1000px', maxHeight: '90vh' }} onClick={e => e.stopPropagation()}>
            <div style={modalHeader}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ background: '#1d9e75', padding: '6px', borderRadius: '6px' }}><Icon name="nozzle" size={16} /></div>
                <div>
                  <h3 style={{ fontSize: '15px', margin: 0 }}>Sales Report: {reportNozzle.name}</h3>
                  <div style={{ fontSize: '10px', color: '#94a3b8' }}>Nozzle Performance & Sales History</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px', alignItems: 'flex-end', background: '#0f151f', padding: '10px', borderRadius: '8px', border: '1px solid #2a3340' }}>
                <div>
                  ...
                  <DatePicker 
                    ...
                    onChange={d => { setReportFromDate(d); fetchReport(reportNozzle.id, d, reportToDate); }} 
                    dateFormat="dd/MM/yy" 
                    className="pp-input" 
                ...
                <div>
                  ...
                  <DatePicker 
                    ...
                    onChange={d => { setReportToDate(d); fetchReport(reportNozzle.id, reportFromDate, d); }} 
                    dateFormat="dd/MM/yy" 
                    className="pp-input" 
                    style={{ ...inputStyle, width: '100px' }} 
                  />
                ...
                <button 
                  onClick={() => fetchReport(reportNozzle.id, reportFromDate, reportToDate)} 
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '10px' }}>
                  ...
                  <tbody>
                    {reportLoading ? (
                      ...
                      <tr><td colSpan="6" style={{ padding: '20px', textAlign: 'center', color: '#6c7f8f' }}>No sales for this nozzle</td></tr>
                    ) : (
...
// Report Modal Styling Helpers (Shared with Attendants)
const modalOverlay = {
